...
inline auto flog2(uint32_t v) -> bitcount_t
{
   return 31 - __builtin_clz(v);
}

inline auto trailingzeros(uint32_t v) -> bitcount_t
{
   return __builtin_ctz(v);
}

inline auto flog2(uint64_t v) -> bitcount_t
{
...
inline auto trailingzeros(uint64_t v) -> bitcount_t
{
   #if UINT64_MAX == ULONG_MAX
...
inline auto flog2(uint8_t v) -> bitcount_t
{
   return flog2(uint32_t(v));
}

inline auto flog2(uint16_t v) -> bitcount_t
{
   return flog2(uint32_t(v));
}

#if __SIZEOF_INT128__
inline auto flog2(__uint128_t v) -> bitcount_t
{
   auto high = uint64_t(v >> 64);
   auto low  = uint64_t(v);
...
inline auto trailingzeros(uint8_t v) -> bitcount_t
{
   return trailingzeros(uint32_t(v));
}

inline auto trailingzeros(uint16_t v) -> bitcount_t
{
   return trailingzeros(uint32_t(v));
}

#if __SIZEOF_INT128__
inline auto trailingzeros(__uint128_t v) -> bitcount_t
{
   auto high = uint64_t(v >> 64);
   auto low  = uint64_t(v);
...
inline auto clog2(UInt v) -> bitcount_t
{
   return flog2(v) + ((v & (-v)) != v);
}

template<typename UInt>
inline auto addwithcarry(UInt x, UInt y, bool carryin, bool *carryout) -> UInt
{
...
template<typename UInt>
inline auto subwithcarry(UInt x, UInt y, bool carryin, bool *carryout) -> UInt
{
   UInt half_result = y + carryin;
   UInt result      = x - half_result;
   *carryout        = (half_result < y) || (result > x);
...
public:
   ...
   constexpr explicit uint_x4(UIntX2 v01)
#if PCG_LITTLE_ENDIAN
      : d{v01, UIntX2(0)}
#else
      : d{UIntX2(0), v01}
   ...
   template<class Integral,
            typename std::enable_if<(std::is_integral<Integral>::value && sizeof(Integral) <= sizeof(UIntX2))>::type
               * = nullptr>
   ...
   friend auto operator*(const uint_x4<U, V> &, const uint_x4<U, V> &) -> uint_x4<U, V>;

   template<typename U, typename V>
   friend auto operator*(const uint_x4<U, V> &, V) -> uint_x4<U, V>;

   template<typename U, typename V>
   friend auto divmod(const uint_x4<U, V> &, const uint_x4<U, V> &) -> std::pair<uint_x4<U, V>, uint_x4<U, V>>;
   ...
   template<typename U, typename V>
   friend auto operator<<(const uint_x4<U, V> &, bitcount_t shift) -> uint_x4<U, V>;
   ...
   template<typename U>
   friend auto operator<<(const uint_x4<U, uint64_t> &, bitcount_t shift) -> uint_x4<U, uint64_t>;
   ...
   template<typename U, typename V>
   friend auto operator==(const uint_x4<U, V> &, const uint_x4<U, V> &) -> bool;
   ...
   template<typename U, typename V>
   friend auto operator~(const uint_x4<U, V> &) -> uint_x4<U, V>;

   template<typename U, typename V>
   friend auto operator-(const uint_x4<U, V> &) -> uint_x4<U, V>;

   template<typename U, typename V>
   friend auto flog2(const uint_x4<U, V> &) -> bitcount_t;

   template<typename U, typename V>
   friend auto trailingzeros(const uint_x4<U, V> &) -> bitcount_t;

#if PCG_64BIT_SPECIALIZATIONS
   template<typename U>
   friend auto flog2(const uint_x4<U, uint64_t> &) -> bitcount_t;

   template<typename U>
   friend auto trailingzeros(const uint_x4<U, uint64_t> &) -> bitcount_t;
#endif

   auto operator*=(const uint_x4 &rhs) -> uint_x4 &
   {
      uint_x4 result = *this * rhs;
      return *this   = result;
   }

   auto operator*=(UIntX2 rhs) -> uint_x4 &
   {
      uint_x4 result = *this * rhs;
      return *this   = result;
   }
   ...
   auto operator>>=(bitcount_t shift) -> uint_x4 &
   {
...
template<typename U, typename V>
auto flog2(const uint_x4<U, V> &v) -> bitcount_t
{
#if PCG_LITTLE_ENDIAN
...
template<typename U, typename V>
auto trailingzeros(const uint_x4<U, V> &v) -> bitcount_t
{
#if PCG_LITTLE_ENDIAN
   for(uint8_t i = 0; i < 4; ++i)
   {
...
auto flog2(const uint_x4<UInt32, uint64_t> &v) -> bitcount_t
{
   return v.d.v23 > 0 ? flog2(v.d.v23) + uint_x4<UInt32, uint64_t>::UINT_BITS * 2 : flog2(v.d.v01);
}

template<typename UInt32>
auto trailingzeros(const uint_x4<UInt32, uint64_t> &v) -> bitcount_t
{
...
template<typename UInt, typename UIntX2>
auto divmod(const uint_x4<UInt, UIntX2> &orig_dividend,
            const uint_x4<UInt, UIntX2> &divisor) -> std::pair<uint_x4<UInt, UIntX2>, uint_x4<UInt, UIntX2>>
{
...
template<typename UInt, typename UIntX2>
auto operator/(const uint_x4<UInt, UIntX2> &dividend, const uint_x4<UInt, UIntX2> &divisor) -> uint_x4<UInt, UIntX2>
{
...
template<typename UInt, typename UIntX2>
auto operator*(const uint_x4<UInt, UIntX2> &a, const uint_x4<UInt, UIntX2> &b) -> uint_x4<UInt, UIntX2>
{
...
template<typename UInt, typename UIntX2>
auto operator*(const uint_x4<UInt, UIntX2> &a, UIntX2 b01) -> uint_x4<UInt, UIntX2>
{
   constexpr auto        UINT_BITS = uint_x4<UInt, UIntX2>::UINT_BITS;
   uint_x4<UInt, UIntX2> r         = {0U, 0U, 0U, 0U};
   bool                  carryin   = false;
...
template<typename UInt32>
auto operator*(const uint_x4<UInt32, uint64_t> &a, const uint_x4<UInt32, uint64_t> &b) -> uint_x4<UInt32, uint64_t>
{
      #if defined(_MSC_VER)
   uint64_t hi;
...
template<typename UInt, typename UIntX2>
auto operator~(const uint_x4<UInt, UIntX2> &v) -> uint_x4<UInt, UIntX2>
{
...
template<typename UInt, typename UIntX2>
auto operator==(const uint_x4<UInt, UIntX2> &a, const uint_x4<UInt, UIntX2> &b) -> bool
{
...
template<typename UInt, typename UIntX2>
auto operator<<(const uint_x4<UInt, UIntX2> &v, const bitcount_t shift) -> uint_x4<UInt, UIntX2>
{
   uint_x4<UInt, UIntX2> r        = {0U, 0U, 0U, 0U};
...
#if PCG_64BIT_SPECIALIZATIONS
template<typename UInt32>
auto operator<<(const uint_x4<UInt32, uint64_t> &v, const bitcount_t shift) -> uint_x4<UInt32, uint64_t>
{
