...
{
   static constexpr auto multiplier() -> uint64_t { return 0xda942042e4dd58b5ULL; }
};
...
public:
   ...
   constexpr auto increment() const -> itype { return itype(reinterpret_cast<uintptr_t>(this) | 1); }

   constexpr auto stream() const -> itype { return increment() >> 1; }

   static constexpr bool can_specify_stream = false;

   static constexpr auto streams_pow2() -> size_t
   {
...
public:
   ...
   static constexpr auto increment() -> itype { return 0; }

   static constexpr bool can_specify_stream = false;

   static constexpr auto streams_pow2() -> size_t { return 0u; }
...
public:
   ...
   static constexpr auto stream() -> itype { return default_increment<itype>::increment() >> 1; }
...
public:
   ...
   constexpr auto increment() const -> itype { return inc_; }

   auto stream() -> itype { return inc_ >> 1; }

   void set_stream(itype specific_seq) { inc_ = (specific_seq << 1) | 1; }

   static constexpr bool can_specify_stream = true;

   static constexpr auto streams_pow2() -> size_t { return (sizeof(itype) * 8) - 1u; }
...
public:
   ...
   static constexpr auto period_pow2() -> size_t { return sizeof(state_type) * 8 - 2 * stream_mixin::is_mcg; }

   // It would be nice to use std::numeric_limits for these, but
   // we can't be sure that it'd be defined for the 128-bit types.

   static constexpr auto min() -> result_type { return result_type(0UL); }

   static constexpr auto max() -> result_type { return result_type(~result_type(0UL)); }

protected:
   auto bump(itype state) -> itype { return state * multiplier() + increment(); }

   auto base_generate() -> itype { return state_ = bump(state_); }

   auto base_generate0() -> itype
   {
...
public:
   ...
   auto operator()(result_type upper_bound) -> result_type { return bounded_rand(*this, upper_bound); }

protected:
   static auto advance(itype state, itype delta, itype cur_mult, itype cur_plus) -> itype;

   static auto
   distance(itype cur_state, itype newstate, itype cur_mult, itype cur_plus, itype mask = ~itype(0U)) -> itype;

   auto distance(itype newstate, itype mask = itype(~itype(0U))) const -> itype
   {
...
public:
   ...
   friend auto operator==(
      ...
      const engine<xtype1, itype1, output_mixin1, output_previous1, stream_mixin_rhs, multiplier_mixin_rhs> &) -> bool;
...
template<typename xtype,
         ...
         typename stream_mixin,
         typename multiplier_mixin>
auto engine<xtype, itype, output_mixin, output_previous, stream_mixin, multiplier_mixin>::advance(
   itype state, itype delta, itype cur_mult, itype cur_plus) -> itype
{
...
template<typename xtype,
         ...
         typename stream_mixin,
         typename multiplier_mixin>
auto engine<xtype, itype, output_mixin, output_previous, stream_mixin, multiplier_mixin>::distance(
   itype cur_state, itype newstate, itype cur_mult, itype cur_plus, itype mask) -> itype
{
...
template<typename xtype,
         ...
         typename stream_mixin_rhs,
         typename multiplier_mixin_rhs>
auto operator-(const engine<xtype, itype, output_mixin, output_previous, stream_mixin_lhs, multiplier_mixin_lhs> &lhs,
...
inline auto operator!=(
   ...
   const engine<xtype, itype, output_mixin, output_previous, stream_mixin_rhs, multiplier_mixin_rhs> &rhs) -> bool
{
...
struct xsh_rs_mixin
{
   static auto output(itype internal) -> xtype
   {
...
struct rxs_mixin
{
   static auto output_rxs(itype internal) -> xtype
   {
      constexpr auto       bits        = bitcount_t(sizeof(itype) * 8);
      constexpr auto       xtypebits   = bitcount_t(sizeof(xtype) * 8);
...
struct rxs_m_xs_mixin
{
   ...
   static auto unoutput(itype internal) -> itype
   {
      constexpr auto       bits   = bitcount_t(sizeof(itype) * 8);
...
struct dxsm_mixin
{
   inline auto output(itype internal) -> xtype
   {
      constexpr auto xtypebits = bitcount_t(sizeof(xtype) * 8);
      constexpr auto itypebits = bitcount_t(sizeof(itype) * 8);
...
struct xsl_rr_rr_mixin
{
   ...
   static auto output(itype internal) -> itype
   {
...
struct inside_out : private baseclass
{
   ...
   static auto external_step(result_type &randval, size_t i) -> bool
   {
   ...
   static auto external_advance(result_type &randval, size_t i, result_type delta, bool forwards = true) -> bool
   {
      state_type state        = baseclass::unoutput(randval);
...
public:
   static constexpr auto period_pow2() -> size_t
   {
      return baseclass::period_pow2() + table_size * extvalclass::period_pow2();
   }

   PCG_ALWAYS_INLINE auto operator()() -> result_type
   {
...
public:
   ...
   friend auto operator==(const extended<table_pow2_, advance_pow2_, baseclass_, extvalclass_, kdd_> &,
                          const extended<table_pow2_, advance_pow2_, baseclass_, extvalclass_, kdd_> &) -> bool;
   ...
   template<typename CharT,
            ...
            typename extvalclass_,
            bool kdd_>
   friend auto operator>>(std::basic_istream<CharT, Traits> &in,
...
template<bitcount_t table_pow2, bitcount_t advance_pow2, typename baseclass, typename extvalclass, bool kdd>
auto operator==(const extended<table_pow2, advance_pow2, baseclass, extvalclass, kdd> &lhs,
                const extended<table_pow2, advance_pow2, baseclass, extvalclass, kdd> &rhs) -> bool
{
...
template<bitcount_t table_pow2, bitcount_t advance_pow2, typename baseclass, typename extvalclass, bool kdd>
inline auto operator!=(const extended<table_pow2, advance_pow2, baseclass, extvalclass, kdd> &lhs,
                       const extended<table_pow2, advance_pow2, baseclass, extvalclass, kdd> &rhs) -> bool
{
   return !operator==(lhs, rhs);
}
